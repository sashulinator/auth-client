...
export function findDependencyIds(ids: string[], entities: Catalog<Entity>): string[] {
 ...
 * @param uniqKeys keys in object which value must to be uniq thus autogenerated
 */
export function copyEntities<T extends Entity>(entities: Catalog<T>, uniqKeys: string[] = []): Catalog<T> {
  return Object.values(entities).reduce<Catalog<T>>((accEntities, entity) => {
...
export function findParent<T extends Entity>(id: string | number, entities: Catalog<T>): T | undefined {
...
export function findEntity<T extends Entity>(id: string | number, entities: Catalog<T>): T {
  const entity = entities[id]
  assertNotUndefined(entity)
  return entity
}
export function findEntities<T extends Entity>(ids: string[], entities: Catalog<T>): Catalog<T> {
  return ids.reduce<Catalog<T>>((acc, id) => {
...
export function removeEntity<T extends Entity>(
  ...
  const entitiesWithoutChildren =
    entityToRemove.children?.reduce<Catalog<T> | undefined>((accEntities, id) => {
      if (accEntities) {
